...
export default function EditBlogPostPage() {
  ...
  useEffect(() => {
    if (postData?.data) {
      ...
      // Set cover image
      if (post.coverImage) {
        setCoverImage([{ url: post.coverImage, alt: "", order: 0 }]);
      }

      // Set OG image
      if (post.ogImage) {
        setOgImage([{ url: post.ogImage, alt: "", order: 0 }]);
      }
    }
  }, [postData, form]);
  ...
  const onSubmit = async (values: BlogPostFormValues) => {
    try {
      ...
      await updateBlogPost.mutateAsync({ id, data: postDataToSubmit });
      router.push("/admin/blog");
    } catch (error) {
      console.error("Greška pri ažuriranju članka:", error);
    }
  };
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col min-h-screen bg-muted/30">
        <AdminHeader title="Uredi članak" />
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <AdminHeader
            ...
            formActions={{
              ...
              submitLabel: updateBlogPost.isPending
                ? "Spremanje..."
                : "Spremi promjene",
            }}
            actions={
                      ...
                      <div className="border border-border rounded-lg p-4 bg-muted/30">
                        ...
                        <div className="text-muted-foreground text-sm">
                          {form.watch("metaDescription") ||
                            watchExcerpt ||
                            "Opis članka će se prikazati ovdje..."}
                        </div>
                      </div>
                    </div>
                  </ContentCard>
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem className="flex items-center gap-2">
                            ...
                            <FormLabel className="!mt-0">Objavljeno</FormLabel>
                          </FormItem>
                        )}
                      />
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                          ...
                          {postData?.data?.publishedAt && (
                            <div>
                              ...
                              <span className="ml-2 font-medium">
                                {new Date(postData.data.publishedAt).toLocaleDateString("bs")}
                              </span>
                            </div>
                          )}
                        </div>
