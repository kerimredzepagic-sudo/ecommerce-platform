...
import { useCareer, useUpdateCareer } from "@/hooks/useAdminApi";
import {
  ImageGallery,
  ProductImage,
...
export default function EditCareerPage() {
  ...
  const { data: careerData, isLoading: isLoadingCareer } = useCareer(id);
  ...
  useEffect(() => {
    if (careerData?.data) {
      ...
      form.reset({
        ...
        expiresAt: career.expiresAt ? career.expiresAt.slice(0, 16) : "",
        order: career.order || 0,
      });

      if (career.coverImage) {
        setCoverImage([{ url: career.coverImage, isPrimary: true }]);
      }
      ...
      setBenefits(career.benefits.length > 0 ? career.benefits : [""]);
    }
  }, [careerData, form]);
  ...
  const onSubmit = async (values: CareerFormValues) => {
    try {
      ...
      await updateCareer.mutateAsync({ id, data: careerData });
  ...
  if (isLoadingCareer) {
    return (
      <div className="flex flex-col min-h-screen bg-muted/30">
        <AdminHeader title="Učitavanje..." showBack />
        <div className="flex-1 flex items-center justify-center">
          <div className="text-muted-foreground">Učitavanje pozicije...</div>
        </div>
      </div>
    );
  }
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <AdminHeader
            ...
            description={careerData?.data?.title || ""}
            showBack
            formActions={{
              isSubmitting: updateCareer.isPending,
              submitLabel: updateCareer.isPending
                ? "Spremanje..."
                : "Spremi promjene",
            }}
          />
              ...
              <div className="space-y-6">
                {updateCareer.isError && (
                  <div className="rounded-lg bg-destructive/10 border border-destructive/20 p-4 text-sm text-destructive">
                    {updateCareer.error?.message ||
                      "Greška pri ažuriranju pozicije"}
                  </div>
                )}
