...
export function useApiServices(filters: { isActive?: boolean } = {}) {
  const params = new URLSearchParams();
  if (filters.isActive !== undefined)
    params.set("isActive", String(filters.isActive));

  return useQuery({
    queryKey: ["admin", "apiServices", filters],
    queryFn: () =>
      adminFetch<{ success: boolean; data: ApiService[] }>(
        `/api-services${params.toString() ? `?${params.toString()}` : ""}`
      ),
  });
}

export function useApiService(id: string) {
  return useQuery({
    queryKey: ["admin", "apiService", id],
    queryFn: () =>
      adminFetch<{ success: boolean; data: ApiService }>(`/api-services/${id}`),
    enabled: !!id && id !== "new",
  });
}

export function useCreateApiService() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (data: ApiServiceInput) =>
      adminFetch<{ success: boolean; data: ApiService }>("/api-services", {
        method: "POST",
        body: JSON.stringify(data),
      }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin", "apiServices"] });
    },
  });
}
...
export function useUpdateApiService() {
  ...
  return useMutation({
    ...
    onSuccess: (_, { id }) => {
      queryClient.invalidateQueries({ queryKey: ["admin", "apiServices"] });
      queryClient.invalidateQueries({ queryKey: ["admin", "apiService", id] });
    },
  });
}
...
export function useDeleteApiService() {
  ...
  return useMutation({
    mutationFn: (id: string) =>
      adminFetch(`/api-services/${id}`, { method: "DELETE" }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin", "apiServices"] });
    },
  });
...
export interface ApiSlotBookingInfo {
  id: string;
...
export interface ApiSlot {
  ...
  // Virtual (computed) - true when bookedCount >= capacity
  isBooked: boolean;
  isBlocked: boolean;
  // Array of booking references
  bookings?: ApiSlotBookingInfo[];
  createdBy?: {
    id: string;
    firstName: string;
    lastName: string;
...
export type AdminApiSlot = ApiSlot;
...
export interface BulkApiSlotInput {
  startDate: string;
  endDate: string;
  startTime: string;
  endTime: string;
...
export interface ApiSlotFilters {
  page?: number;
  limit?: number;
  from?: string;
  to?: string;
...
export function useApiSlots(filters: ApiSlotFilters = {}) {
  const params = new URLSearchParams();
  if (filters.page) params.set("page", String(filters.page));
  if (filters.limit) params.set("limit", String(filters.limit));
  ...
  if (filters.isBooked !== undefined)
    params.set("isBooked", String(filters.isBooked));
  if (filters.isBlocked !== undefined)
    params.set("isBlocked", String(filters.isBlocked));

  return useQuery({
    queryKey: ["admin", "apiSlots", filters],
    queryFn: () =>
      adminFetch<{
        success: boolean;
        data: ApiSlot[];
...
export function useCreateApiSlot() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (data: ApiSlotInput) =>
      adminFetch<{ success: boolean; data: ApiSlot }>("/api-slots", {
        method: "POST",
        body: JSON.stringify(data),
      }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin", "apiSlots"] });
    },
  });
}

export function useCreateBulkApiSlots() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (data: BulkApiSlotInput) =>
      adminFetch<{
        success: boolean;
...
export function useUpdateApiSlot() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: ({ id, data }: { id: string; data: Partial<ApiSlotInput> }) =>
      adminFetch<{ success: boolean; data: ApiSlot }>(`/api-slots/${id}`, {
        method: "PUT",
        body: JSON.stringify(data),
      }),
    onSuccess: () => {
...
export function useToggleApiSlotBlock() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: ({ id, isBlocked }: { id: string; isBlocked: boolean }) =>
      adminFetch<{ success: boolean; data: ApiSlot }>(
        `/api-slots/${id}/block`,
        {
          method: "PATCH",
...
export function useDeleteApiSlot() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (id: string) =>
      adminFetch(`/api-slots/${id}`, { method: "DELETE" }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin", "apiSlots"] });
    },
  });
...
export interface ApiBookingSession {
  id?: string;
  slotId: string;
  date: string;
  startTime: string;
...
export interface ApiBooking {
  ...
  isPackageInitialBooking?: boolean;
  packageCode?: {
    usedSessions: number;
...
// Alias for admin page usage
export type AdminApiBooking = ApiBookingListItem;

export interface ApiBookingFilters {
  page?: number;
  limit?: number;
  status?: string;
  paymentStatus?: string;
...
export function useApiBookings(filters: ApiBookingFilters = {}) {
  const params = new URLSearchParams();
  if (filters.page) params.set("page", String(filters.page));
  if (filters.limit) params.set("limit", String(filters.limit));
  if (filters.status) params.set("status", filters.status);
  ...
  return useQuery({
    queryKey: ["admin", "apiBookings", filters],
    queryFn: () =>
      adminFetch<{
        success: boolean;
...
export function useApiBooking(id: string) {
  return useQuery({
    queryKey: ["admin", "apiBooking", id],
    queryFn: () =>
      adminFetch<{ success: boolean; data: ApiBooking }>(
...
export function useUpdateApiBookingStatus() {
  const queryClient = useQueryClient();
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin", "apiBookings"] });
    },
  });
}

export function useUpdateApiBookingPayment() {
  const queryClient = useQueryClient();
...
export function useUpdateApiBookingSessionStatus() {
  const queryClient = useQueryClient();
  ...
  return useMutation({
    mutationFn: ({
      ...
      adminFetch<{ success: boolean; data: ApiBooking }>(
        `/api-bookings/admin/${bookingId}/session/${sessionIndex}/status`,
        {
          method: "PATCH",
          body: JSON.stringify({ status }),
        }
...
export function useCancelApiBookingAdmin() {
  const queryClient = useQueryClient();
...
export function useDeleteApiBooking() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (id: string) =>
      adminFetch(`/api-bookings/admin/${id}`, { method: "DELETE" }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin", "apiBookings"] });
    },
  });
